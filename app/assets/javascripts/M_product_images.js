var new_img_field = 
'<div class="M-img-field"><button class="btn btn-danger M-btn-image"><i class="fa fa-plus-circle fa-3x"></i> </br>Add Image</button><input name="images[url][]" class="M-image-input" onchange="previewImages(event)" type="file" id="post_images_attributes_0_url"><div class="M-image-output"><div class="btn btn-light M-image-remove" onclick="removeImages(event)"><i class="fa fa-trash"></i></div></div></div>'
...
function showImageInput() {
  if ($('#product_img_field_container').children('.M-img-field').length < 4)
